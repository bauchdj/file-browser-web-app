...
const zip = require('./routes/zip.js');
...
function getFiles(ws, data) {
	getListOfFiles(data.path, (err, fileList) => {
		if (err) return ws.send(JSON.stringify({ success: false, error: "FROM BACKEND\n" + err.toString() }));
		ws.send(JSON.stringify({ success: true, type: data.type, data: fileList }));
	});
}
...
module.exports = function (ws, data) {
	...
	if (data.type === "fileExt to class") {
		fs.readFile(__dirname + "/fileType-to-FontawesomeClass.json", "utf8", (error, data) => {
			if (error) return console.log(error);
			ws.send(data);
		});
	}

	if (data.type === "list dir") {
		data.path = usersPath + data.path; // Gets directory of user
		getFiles(ws, data);
	}
}
/*
